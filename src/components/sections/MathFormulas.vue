  ...
  <div class="mt-12 fade-in">
    <div class="control-card p-8 max-w-5xl mx-auto">
      <div class="text-center mb-8">
        ...
        <h3 class="text-3xl font-bold bg-gradient-to-r from-indigo-900 via-purple-800 to-indigo-900 bg-clip-text text-transparent">
          Mathematical Formulas
        </h3>
        <p class="text-gray-600 mt-2">Understanding the mathematics behind activation functions</p>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="flex items-center mb-3">
            <component :is="formula.icon" class="w-5 h-5 text-indigo-600 mr-2" />
            <h4 class="font-semibold text-gray-800">{{ formula.name }}</h4>
          </div>
          <div class="text-gray-700 mb-2" v-html="formula.latex"></div>
          <p class="text-sm text-gray-600">{{ formula.description }}</p>
        </div>
      </div>
    </div>
  </div>
